...
function data = parseString(input)
% Source of stopwords- http://norm.al/2009/04/14/list-of-english-stop-words/
...
delimiters = {'>','<','.','.','''',',','-',':','?','"','(',')','[',']','#','{','}',';',...
   ...
   '?','?','?','?','?','ã','?','?'};
cells = cellstr(str1);
tokenize = @(x) textscan(x, '%s', 'Delimiter', delimiters);
tokenized = cellfun(tokenize, cells);
str1 = strjoin(tokenized{1:end},' ');
str1 = lower(str1);
...
split1(:,length(split1) < 2) = [];
split1 = cellfun(@porterStemmer, split1, 'UniformOutput', false);
% data = strjoin(split1(~ismember(split1,stopwords_cellstring)),' ');
data = strjoin(split1,' ');
end